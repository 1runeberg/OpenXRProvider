...
#include "DiligentBase.hpp"

namespace Diligent
{

void DiligentBase::ModifyEngineInitInfo(const ModifyEngineInitInfoAttribs& Attribs)
{
...
float4x4 DiligentBase::GetAdjustedProjectionMatrix(float FOV, float NearPlane, float FarPlane) const
{
...
float4x4 DiligentBase::GetSurfacePretransformMatrix(const float3& f3CameraViewAxis) const
{
    const auto& SCDesc = m_pSwapChain->GetDesc();
